...
export const EN = {
  ...
  '小さな部屋': 'Small room', '室内楽ホール': 'Chamber hall', '大ホール': 'Concert hall', '石の教会': 'Stone church',
  // 録音証明（_録音証明.txt）
  '録音証明': 'Provenance', '_録音証明': '_provenance', '_素': '_raw', '_仕上げ': '_finished', '仕上げの覚え書き': 'Finishing notes', '素（加工ゼロのミックス）': 'raw (untouched mix)',
  'この録音は、次の経路で録られました。素の WAV には録音後いっさい手を加えていません。': 'This recording was captured through the path below. The raw WAV has not been touched after recording.',
  '{sec} 秒・{khz} kHz・{ch}ch': '{sec} s · {khz} kHz · {ch}ch', '・開始 {sec} 秒': ' · starts at {sec} s',
  '（証拠なし：以前の版で録ったテイク）': '(no evidence: take recorded with an earlier version)',
  '録った時刻：': 'Recorded at: ', '入り口：': 'Input: ', 'keyboard のアプリ音バス（別トラック）': 'keyboard app-audio bus (side track)', '道：': 'Path: ',
  '生フレーム取得（AudioContext を通らない。再標本化なし）': 'raw frame capture (bypasses AudioContext, no resampling)', 'AudioContext 経由': 'via AudioContext', '（⚠ 再標本化あり）': ' (⚠ resampled)',
  '置いた位置：本線の先頭から {sec} 秒（押す前の音＋奏者が聞くまでの遅れ）': 'Placed at {sec} s from the start of the main take (pre-roll + the delay before the player hears it)',
  'ブラウザの加工：': 'Browser processing: ', 'すべて切': 'all off', '不明（ブラウザが答えない）': 'unknown (browser does not report)', '⚠ 残っている': '⚠ still on',
  '届いたビット数：': 'Bits received: ', 'float（整数の刻みに乗っていない）': 'float (not on an integer grid)', '未確定': 'undetermined',
  '経路の検証：': 'Path verification: ', '2つの道で 1 サンプルも違わず一致': 'both paths identical to the sample', '暗騒音：': 'Noise floor: ',
  '落ちた音：{n} 回': 'Dropouts: {n}', '（{n} フレームを無音で埋めて長さを保った）': ' ({n} frames filled with silence to keep the length)',
  '押す前の音：{sec} 秒を先頭に含む': 'Pre-roll: {sec} s included at the start', 'クロックのずれ：': 'Clock drift: ', '穴の位置：': 'Dropout positions: ', 'クリック：{n} か所': 'Clicks: {n}',
  '受け皿：': 'Storage: ', 'OPFS へ同期追記': 'synchronous append to OPFS', '＋フォルダ直書き': ' + direct folder write',
  // 仕上げの覚え書き（_仕上げ.txt）
  '素（_素.wav）には何も足していません。この仕上げは、素に次のものを通した音です。': 'Nothing was added to the raw file (_raw.wav). This finished version is the raw take passed through the following:',
  '・風音カット：30 Hz 以下をハイパス（Q 0.707）': '· Rumble cut: high-pass below 30 Hz (Q 0.707)',
  '電源ハム除去 {hz} Hz とその倍音 ×{n}（ノッチ Q 30）': 'hum removal at {hz} Hz and {n} harmonics (notch Q 30)', 'ノイズゲート しきい値 {db} dBFS': 'noise gate, threshold {db} dBFS', '響きの量 {pct}%': 'reverb send {pct}%',
  '・ホールの響き：{hall}（鏡像法の初期反射＋3帯域の後部残響）／響きの長さ {sec} 秒／量 {pct}%／直接音から響きまで {ms} ms': '· Hall reverb: {hall} (image-source early reflections + 3-band late tail) / decay {sec} s / amount {pct}% / pre-delay {ms} ms',
  '直接音には触っていません。響きだけを足しています。': 'The direct sound is untouched; only the reverb is added.',
  '・音量そろえ：いちばん大きいところ（True Peak）を {db} dBTP に': '· Level match: loudest point (true peak) set to {db} dBTP', '盛り度：': 'Finishing amount: ',
  '（音の変化＝音量を合わせたあとに残る差。−30 dB より下なら耳ではほぼ分からない）': '(sound change = the residual after matching level; below −30 dB it is practically inaudible)',
